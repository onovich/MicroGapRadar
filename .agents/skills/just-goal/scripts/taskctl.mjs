...
const validStatuses = new Set(["DRAFT", "READY", "IN_PROGRESS", "REVIEW", "REQUEST_CHANGES", "ACCEPTED", "CLOSED", "BLOCKED", "PARTIAL"]);
...
const mutatingCommands = new Set(["init", "create", "import", "set-status", "set-thread", "clear-thread", "archive"]);

try {
  if (mutatingCommands.has(command)) ensureDirs();
