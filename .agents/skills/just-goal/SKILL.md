---
name: just-goal
description: Generic multi-agent project orchestration for software and R&D repositories. Use when a project needs dynamic subagents, scoped tasks, plan-execute-test-review loops, structured handoffs, autonomous milestone execution, reviewer gates, model routing initialization, and leadership handoff. Works across domains and agent platforms that support skills or skill-like instructions; configure project-specific docs, milestones, roles, models, and human gates in the repository.
---
...
Using this Skill is authorization to use its normal orchestration machinery: spawn writer/reviewer subagents when native multi-agent tools are available, route handoffs with native thread/message tools such as `send_input`, create the next scoped task when the queue is empty, and run tasks through `EXECUTE` or `AUTONOMOUS_GOAL` mode. Do not ask for separate permission merely to spawn subagents, continue goal mode, create the next ordinary task from accepted project docs, or execute a dependency-ready task that already satisfies this Skill's gates.
...
4. Initialize orchestration files only if absent. Replace `<SKILL_PATH>` with the installed Skill directory, for example `.agents/skills/just-goal` in a repository install:

```bash
node <SKILL_PATH>/scripts/taskctl.mjs init --repo-root .
```

5. Initialize model routing before assigning roles. Use native model-listing or account/config inspection when available; otherwise ask the human for the available model names and capabilities, then record them:

```bash
node <SKILL_PATH>/scripts/modelctl.mjs init --repo-root .
node <SKILL_PATH>/scripts/modelctl.mjs record --repo-root . --models "model-a,model-b" --deep "model-a" --balanced "model-b" --fast "model-b"
```

6. Never overwrite uncommitted human work. Never `reset --hard`, `git clean -fd`, force-push, or delete unknown files unless the human explicitly authorizes that exact action.
...
- `MODEL_INIT`: discover available models and write `project/model-routing-state.json` before role planning.
...
## Model routing initialization

Do not assume a specific model name is available. Before spawning or assigning agents, create or refresh `project/model-routing-state.json` with the best available models for these capability tiers:

- `deep`: strongest reasoning model available; use for lead, architecture, domain, QA, and security review.
- `balanced`: reliable implementation model; use for ordinary writer roles and test work.
- `fast`: low-latency economical model; use for research scans, mechanical release work, and tightly scoped fast-worker tasks.

Prefer the user's configured model names over examples in this Skill. If native model discovery is unavailable, ask for the available models once, record the mapping, and continue. If no routing state exists, use the current session model for all roles and record that fallback before execution.

Useful commands:

```bash
node <SKILL_PATH>/scripts/modelctl.mjs init --repo-root .
node <SKILL_PATH>/scripts/modelctl.mjs record --repo-root . --models "comma,separated,models" --deep "<deep-model>" --balanced "<balanced-model>" --fast "<fast-model>"
node <SKILL_PATH>/scripts/modelctl.mjs validate --repo-root .
```

## Role defaults

Use these defaults unless the repository overrides them in agent profile files, `project/model-routing-state.json`, or project docs.

| Role | Model tier | Effort | Sandbox | Use |
|---|---|---:|---|---|
| `lead_orchestrator` | `deep` | `xhigh` | write | sole router, task owner, integration gate |
| `systems_architect` | `deep` | `xhigh` | write/review | architecture, public APIs, package boundaries |
| `domain_designer` | `deep` | `xhigh` | write/review | product/domain rules and cross-system design |
| `implementation_engineer` | `balanced` | `high` | write | core implementation |
| `client_engineer` | `balanced` | `high` | write | UI/client/runtime integration |
| `data_content_engineer` | `balanced` | `high` | write | schemas, fixtures, content/data pipelines |
| `test_engineer` | `balanced` | `high` | write | tests, fixtures, coverage, regression checks |
| `qa_reviewer` | `deep` | `xhigh` | read-only | independent acceptance reviewer |
| `security_reviewer` | `deep` | `xhigh` | read-only | secrets, IPC, supply chain, permissions |
| `research_scout` | `fast` | `medium` | read-only | broad scans, evidence collection |
| `release_engineer` | `fast` | `medium` | write | CI, packaging, scripts, mechanical release tasks |
| `spark_worker` | `fast` | `medium` | write | small scoped implementation, never final review |

Fast-worker roles are executors, not decision makers. They require `ALLOWED_PATHS`, `FORBIDDEN_PATHS`, `REQUIRED_TESTS`, `STOP_CONDITIONS`, and a `deep` reviewer.
...
- `REQUEST_CHANGES`: reviewer has rejected the current handoff and the task must be routed back to the writer or transferred by the lead.
...
```bash
node <SKILL_PATH>/scripts/taskctl.mjs set-thread <TASK_ID> <ROLE> <THREAD_ID> --repo-root .
```
...
```bash
node <SKILL_PATH>/scripts/validate-handoff.mjs <handoff.json>
```
...
```bash
node <SKILL_PATH>/scripts/render-route-message.mjs <handoff.json>
```
...
12. **Continue**
   ...
   - Run `node .agents/skills/just-goal/scripts/verify-autonomous-stop.mjs` before any final response.
